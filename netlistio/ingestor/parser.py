...
# A single picklable unit of work handed to a multiprocessing worker.
_WorkItem = tuple[str, ParseRegion, "ChunkParserFactory"]


def _worker_entry_point(args: _WorkItem) -> ParseResult:
    """
...
class ChunkParser(abc.ABC):
    ...
    def __iter__(self) -> Generator[str, Any, Any]:
        ...
        Implementations must not yield empty strings or None.

        :return: Iterator yielding non-empty logical lines as strings.
        """
...
class Parser:
    ...
    def __init__(self, filepath: str | Path, scanner: Iterable[ParseRegion], chunk_parser_factory: ChunkParserFactory):
        """
